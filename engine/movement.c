...
/*   Updated: 2021/03/08 07:04:54 by ccastill         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	left_right(t_cub *cub, t_player *player)
{
	if (cub->k.left)
	{
		if (can_move(g_check.map[(int)player->pos_x][(int)(player->pos_y + player->dir_x * SPEED)]))
			player->pos_y += player->dir_x * SPEED;
		if (can_move(g_check.map[(int)(player->pos_x - player->dir_y * SPEED)][(int)player->pos_y]))
...
int    move_fow_back(t_cub *cub, t_player *player)
{
	if (cub->k.up)
	{
		if (can_move(g_check.map[(int)(player->pos_x + player->dir_x * SPEED)][(int)player->pos_y]))
			player->pos_x += player->dir_x * SPEED;
		if (can_move(g_check.map[(int)player->pos_x][(int)(player->pos_y + player->dir_y * SPEED)]))
